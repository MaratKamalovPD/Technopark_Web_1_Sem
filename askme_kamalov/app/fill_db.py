...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        names = requests.get(url=f'https://names.drycodes.com/{ratio}?nameOptions=boy_names').json()
        ...
        for user in users:
            for i in range(10):
                ...
                question = Question(owner=user, title=title, content=content)
        ...
        for question in questions:
            for j in range(10):
                ...
                answer = Answer(owner=owner, question=question, content=ans_content)
        ...
        bar = IncrementalBar('Likes added', max=len(answers))
